;(function($){
	var methods = {
		...
		pageData: function(_v, _current){
			...
			if(opts.ajaxSetData&&(typeof(opts.ajaxSetData)==='boolean')){
				if(opts.url!=='' && typeof(opts.url)==='string'){
					ajaxOpts = methods.ajaxData(opts.url, _current, opts.pageData, opts.companyName, opts.type);
		...
		ajaxData: function(_url, _current, pageSize, companyName, type){
			/**
			...
			var _pageData = $.fn.zPager.defaults.pageData;
			return (function(){
				var parms = {'total':_total,'rows':[]};
				$.ajax({
					url: _url,
					type: 'get',
					data: {"page":_current, "pageSize": pageSize || _pageData, "companyName": companyName, "type": type},
	                dataType: 'json',
	                cache : false,  
        			async : false,
